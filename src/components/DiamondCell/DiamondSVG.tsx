...
const DiamondSVG: React.FC<DiamondSVGProps> = ({ cellResult, bases, scored }) => (
  <svg viewBox="0 0 100 100" style={{ width: "100%", height: "100%" }}>
    <polygon points="50,5 95,50 50,95 5,50" fill={cellResult === "HR" || scored ? "limegreen" : "#fff"} stroke="#000" strokeWidth="2" />
    {["1B", "BB", "2B", "3B"].includes(cellResult) && (
      <polygon points={hitPolygons[cellResult]} fill="limegreen" stroke="limegreen" strokeWidth={4} opacity={0.8} />
    )}
    {["K", "ꓘ"].includes(cellResult) && (
      <text x="50" y="55" textAnchor="middle" dominantBaseline="middle" fontSize="40" fontWeight="bold" fill="red">
        {cellResult}
      </text>
    )}
